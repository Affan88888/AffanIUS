...
class RegisterPage {
  ...
  async fillRegistrationForm(
    ...
    // Captcha text
    const captchaImageText = await page.getAttribute('#captcha', 'src') || '';
    const extractedCaptchaText = captchaImageText.match(/captcha=([a-zA-Z0-9]+)/);
    const captchaTextToFill = extractedCaptchaText ? extractedCaptchaText[1] : '';
    await page.fill('#captcha-form', captchaTextToFill);
